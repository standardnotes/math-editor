...
(function (document, window) {
	...
	function ParserCollection(defaults, imageLoader, markdownit, setResultView, sourceGetter, sourceSetter, domSetPreviewHTML, domSetHighlightedContent, updateCallback) {
		...
		var _mdHtmlAndTex = markdownit(defaults).use(markdownitS2Tex, { noreplace: true }).use(markdownitSub).use(markdownitSup);
  ...
  */
	...
	documentReady(function () {
		...
		}), eNodeSource, eResultHtml, document.querySelector('[id^="container-block"]'));
	...
	w.markdownitS2Tex = function math_plugin(md, options) {
		...
		md.renderer.rules.math_inline = function (protocol) {
			...
			return function (tokens, idx) {
				...
				return isInline ? '<img src="' + url + '" alt="' + md.utils.escapeHtml(formula) + '" />' : '<img align="center" src="' + url + '" alt="' + md.utils.escapeHtml(formula) + '" />';
			};
